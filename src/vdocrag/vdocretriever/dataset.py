...
class TrainDataset(Dataset):
    def __init__(self, data_args: DataArguments, trainer = None):
        ...
        for relevant_doc_ids in tqdm(self.train_data['relevant_doc_ids']):
            qa_rel_doc_ids.add(relevant_doc_ids[0])
        for qa_rel_doc_id in qa_rel_doc_ids:
            if qa_rel_doc_id not in self.docid2idx:
                count+=1
        print(count)
        print(len(qa_rel_doc_ids))
        # print('docvqa/mqvb0228_33.png' in self.docid2idx)
    ...
    def __getitem__(self, item) -> Tuple[str, List[str]]:
        ...
        if self.data_args.pretrain:
            ...
            summary=self.doc_id2summary[docid]
            image = image = self.corpus[self.docid2idx[docid]]['image']

        return query, image,summary
